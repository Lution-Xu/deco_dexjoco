...
class DECOTextUnify(nn.Module):
    ...
    def img_encoding(self, images, camera_mask):
        ...
        feat = outputs.last_hidden_state.to(dtype=images.dtype)
        feat = self._drop_non_spatial_token_if_needed(feat)
        seq_len = feat.shape[1]
        feat_h = int(math.sqrt(seq_len))
        feat_w = feat_h
        if feat_h * feat_w != seq_len:
            raise ValueError(f"SigLIP token count {seq_len} cannot be reshaped to a square grid")
