...
class DECOTextUnify(nn.Module):
    ...
    def img_encoding(self, images, camera_mask):
        ...
        feat_h = valid_images.shape[-2] // patch_size
        feat_w = valid_images.shape[-1] // patch_size
        ...
        with vision_context:
            outputs = self.vision_encoder(
                pixel_values=valid_images.to(dtype=vision_dtype),
                return_dict=True,
            )
        tokens = outputs.last_hidden_state.to(dtype=images.dtype)
